...
const careCard = (icon: React.ReactNode, label: string, value: string) => (
    <div style={{ background: '#F0F7F0', padding: '14px', borderRadius: '16px', flex: '1 1 calc(50% - 6px)' }}>
        <div style={{ marginBottom: '6px' }}>{icon}</div>
        <div style={{ fontSize: '10px', fontWeight: '700', color: '#1A4D2E', opacity: 0.5, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>{label}</div>
        <div style={{ fontSize: '13px', fontWeight: '600', color: '#1A4D2E' }}>{value}</div>
    </div>
);

export const PlantDetail: React.FC<PlantDetailProps> = ({ plant, onClose, onSave, isInGarden }) => {
    return (
        <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 110, background: 'white', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
        >
            {/* Image Header */}
            ...
            {/* Content Sheet */}
            <div style={{ background: 'white', borderRadius: '28px 28px 0 0', marginTop: '-20px', padding: '24px 20px', position: 'relative', zIndex: 10, flex: 1 }}>
                {/* About */}
                <section style={{ marginBottom: '24px' }}>
                    <h2 style={{ fontSize: '17px', fontWeight: '800', color: '#1A4D2E', marginBottom: '8px' }}>About</h2>
                    <p style={{ color: '#4E342E', opacity: 0.7, lineHeight: 1.6, fontSize: '14px', margin: 0 }}>{plant.description}</p>
                </section>

                {/* Care Guide */}
                <section style={{ marginBottom: '28px' }}>
                    <h2 style={{ fontSize: '17px', fontWeight: '800', color: '#1A4D2E', marginBottom: '12px' }}>Care Guide</h2>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                        {careCard(<Droplets size={20} color="#1976D2" />, 'Watering', plant.care.water)}
                        {careCard(<Sun size={20} color="#F9A825" />, 'Sunlight', plant.care.light)}
                        {careCard(<Thermometer size={20} color="#e64a19" />, 'Temperature', plant.care.temperature)}
                        {careCard(<AlertTriangle size={20} color="#c62828" />, 'Toxicity', plant.toxicity)}
                    </div>
                </section>

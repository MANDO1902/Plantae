    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '52px 20px 16px', background: 'white', borderBottom: '1px solid #E0EDE0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                <h2 style={{ fontSize: '22px', fontWeight: '800', color: '#1A4D2E', margin: 0 }}>Scan History</h2>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }} className="no-scrollbar">
                {history.length === 0 ? (
                    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', color: '#4E342E', opacity: 0.4 }}>
                        <div style={{ width: '72px', height: '72px', background: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <Leaf size={36} />
                        </div>
                        <p style={{ margin: 0, fontSize: '16px', fontWeight: '500' }}>No scans yet. Start scanning!</p>
                    </div>
                ) : (
                    history.map((item, i) => (
                        <motion.div
                            ...
                            style={{ background: 'white', padding: '12px', borderRadius: '18px', display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '10px', cursor: 'pointer', boxShadow: '0 2px 8px rgba(26,77,46,0.05)' }}
                        >
                            <img
                                src={item.image}
                                alt={item.commonName}
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <h3 style={{ fontWeight: '700', fontSize: '15px', color: '#1A4D2E', margin: '0 0 3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.commonName}</h3>
                                <p style={{ fontSize: '12px', color: '#4E342E', opacity: 0.55, fontStyle: 'italic', margin: '0 0 8px' }}>{item.scientificName}</p>
                                <div style={{ display: 'flex', gap: '12px', fontSize: '11px', color: '#4E342E', opacity: 0.4, fontWeight: '600' }}>
